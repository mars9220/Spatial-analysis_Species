...
layer1 = gpd.read_file(
    "/Users/jue-ying/Desktop/Data-Science-Class/GIS/物種多樣性/spaitial-total-count.geojson"
)
layer2 = gpd.read_file("/Users/jue-ying/Desktop/Data-Science-Class/GIS/物種多樣性/TSMC_Buffered_all.geojson")

# 2. 執行空間相交 (Spatial Join)
joined_data = gpd.sjoin(layer1, layer2, how="inner", predicate="intersects")

# 3. 儲存相交後的 GeoJSON
joined_data.to_file("joined_data.geojson", driver="GeoJSON")

# 4. 讀取剛剛生成的 GeoJSON
file_path = "joined_data.geojson"
...
if all_results:
    ...
    # 11. 儲存為 CSV
    final_df.to_csv("joined-company-species.csv", index=False, encoding="utf-8")
    print(f"✅ 結果已儲存至 'joined-company-species.csv'，共 {len(final_df)} 筆資料。")
else:
    print("⚠️ 沒有符合條件的資料！")
